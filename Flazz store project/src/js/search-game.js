...
const cariGames = function () {
  if (inputSearch.value === '') return;

  const cari = inputSearch.value.toLowerCase().split(' ').join('');

  const games = datass.filter(game =>
    game.nama.toLowerCase().split(' ').join('').startsWith(cari)
  );

  const markup = games
    .map(game => {
      return `<ul class="result-item">
            <a href="Topup.html#${game.id}" class="result-link">
...
const toggleSearch = function () {
  overlaySearch.classList.toggle('hidden');
  searchContainer.classList.toggle('hidden');
};

inputSearch.addEventListener('input', cariGames);
labelSearch.addEventListener('click', cariGames);

[overlaySearch, searchBtn].forEach(el =>
